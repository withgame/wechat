...
type Client struct {
	AccessTokenServer *DefaultComponentAccessTokenServer
	HttpClient        *http.Client
}

// NewClient 创建一个新的 Client.
//  如果 clt == nil 则默认用 util.DefaultHttpClient
func NewClient(srv *DefaultComponentAccessTokenServer, clt *http.Client) *Client {
	if srv == nil {
...
//          ...
//      }
func (clt *Client) Token() (string, error) {
	return clt.AccessTokenServer.Token()
}

func (clt *Client) RefreshToken(token string) (string, error) {
	return clt.AccessTokenServer.RefreshToken(token)
}
